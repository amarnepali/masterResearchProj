...
def main():
    ...
    def plot_reference_points(reference_points):
        ...
        plt.savefig(os.path.join('DATA_OUT', 'reference_points_DHM_ILC_50'))
    ...
    def plot_population(population):
        ...
        # plt.legend()
        # plt.show()
        plt.savefig(os.path.join('DATA_OUT', 'final_population_fitness_DHM_ILC_50'))
    ...
    def plot_fitness_values_per_individual(fitness_values):
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join('DATA_OUT', 'revisit_time_per_individuals_DHM_ILC_50'))
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join('DATA_OUT', 'dwell_time_per_individuals_DHM_ILC_50'))
    ...
    def dynamic_crossover_mutation(gen, ngen):
        ...
        """
        Pc = gen / ngen ## Crossover increases linearly from 0% to 100%
        Pm = 1.0 - (gen / ngen)   # # Mutation decreases linearly from 100% to 0%
        return Pc, Pm

     # Save the top 10 best final indivudal of population to a file
    def save_best_individuals(population, filename="best_individuals_DHM_ILC_50.csv"):
    ...
    def save_best_10_fitness_per_gen(population, gen, filename="best_fitness_DHM_ILC_50.csv"):
        """
    ...
    def run_ga_mu_plus_lambda_dynamic(initial_mu, lambda_, ngen, min_mu, decrease_rate):
        ...
        plt.savefig(os.path.join('DATA_OUT', 'pop_size_over_generation_DHM_ILC_50'))
        ...
        # plt.legend()
        # plt.show()
        plt.savefig(os.path.join('DATA_OUT', 'pareto_front_DHM_ILC_50'))
