...
def main():
    ...
    def plot_reference_points(reference_points):
        ...
        plt.savefig(os.path.join('DATA_OUT', 'reference_points_ILM_DHC_50'))
    ...
    def plot_population(population):
        ...
        # plt.legend()
        # plt.show()
        plt.savefig(os.path.join('DATA_OUT', 'final_population_fitness_ILM_DHC_50'))
    ...
    def plot_fitness_values_per_individual(fitness_values):
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join('DATA_OUT', 'revisit_time_per_individuals_ILM_DHC_50'))
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join('DATA_OUT', 'dwell_time_per_individuals_ILM_DHC_50'))
    ...
    def dynamic_crossover_mutation(gen, ngen):
        ...
        - Starts with 0% mutation and 100% crossover.
        - Gradually decreases mutation rate and increases crossover rate over generations.
        - At the last generation, mutation is 100% and crossover is 0%.

        This ensures the algorithm initially focuses on exploitation (crossover), 
        but as it progresses, the focus shifts toward exploration (mutation) in later generations
        """
        Pc = 1.0-(gen / ngen) ## Crossover rate decreases linearly from 100% to 0%
        Pm = gen / ngen  # # Mutation rate increases linearly from 0% to 100%
        return Pc, Pm

     # Save the top 10 best final indivudal of population to a file
    def save_best_individuals(population, filename="best_individuals_ILM_DHC_50.csv"):
    ...
    def save_best_10_fitness_per_gen(population, gen, filename="best_fitness_ILM_DHC_50.csv"):
        """
    ...
    def run_ga_mu_plus_lambda_dynamic(initial_mu, lambda_, ngen, min_mu, decrease_rate):
        ...
        plt.savefig(os.path.join('DATA_OUT', 'pop_size_over_generation_ILM_DHC_50'))
        ...
        # plt.legend()
        # plt.show()
        plt.savefig(os.path.join('DATA_OUT', 'pareto_front_ILM_DHC_50'))
    ...
    run_ga_mu_plus_lambda_dynamic(initial_mu=50, lambda_=50, ngen=100, min_mu=40, decrease_rate=1)  # Example population of 20 and 10 generations lmbda_>= mu
