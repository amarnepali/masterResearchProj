...
def main():
    ...
    def plot_reference_points(reference_points):
        ...
        plt.savefig('reference_points_ILM_DHC_LHA')
    ...
    def plot_population(population):
        ...
        # plt.legend()
        # plt.show()
        plt.savefig('final_population_fitness_ILM_DHC_LHA')
    ...
    def plot_fitness_values_per_individual(fitness_values):
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig( 'revisit_time_per_individuals_ILM_DHC_LHA')
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig( 'dwell_time_per_individuals_ILM_DHC_LHA')
    ...
    def dynamic_crossover_mutation(gen, ngen):
        ...
        """
        Pc = 1-(gen / ngen) ## Crossover decreases linearly 
        Pm = (gen / ngen)   # # Mutation increases linearly 
        return Pc, Pm

     # Save the top 10 best final indivudal of population to a file
    def save_best_individuals(population, filename="best_individuals_ILM_DHC_LHA.csv"):
    ...
    def save_best_10_fitness_per_gen(population, gen, filename="best_fitness_ILM_DHC_LHA.csv"):
        """
    ...
    def run_ga_mu_plus_lambda_dynamic(initial_mu, lambda_, ngen, min_mu, decrease_rate):
        ...
        plt.savefig( 'pop_size_over_generation_ILM_DHC_LHA')
        ...
        # plt.legend()
        # plt.show()
        plt.savefig('pareto_front_ILM_DHC_LHA')
