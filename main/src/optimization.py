...
def get_footprint(propagator, date, earth):
    ...
    fov = CircularFieldOfView(losPointing_body, radians(1.5),  radians(0.0))       #just taking the center as nadir point as it is giving a  circular footprint, need to look further about the axes of spacecraft and sensor 
...
def main():
    ...
    def plot_reference_points(reference_points):
        ...
        plt.savefig('reference_points_DHM_ILC_LHA')
    ...
    def plot_population(population):
        ...
        # plt.legend()
        # plt.show()
        plt.savefig('final_population_fitness_DHM_ILC_LHA')
    ...
    def plot_fitness_values_per_individual(fitness_values):
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig( 'revisit_time_per_individuals_DHM_ILC_LHA')
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig( 'dwell_time_per_individuals_DHM_ILC_LHA')
    ...
    def random_inclination():
        return np.random.uniform(120, 170)
    ...
    def mutGaussianPlanes(individual, mu, sigma, indpb):
        for i in range(len(individual[0])):  # Iterate over each plane
            if np.random.random() < indpb:
                ...
                mutated_inclination = np.clip(inclination + np.random.normal(mu, sigma), 20, 90)
    ...
    def dynamic_crossover_mutation(gen, ngen):
        """
        Implements the DHM/ILC (Dynamic Decreasing High Mutation and Increasing Low Crossover) strategy.
        - Starts with 100% mutation and 0% crossover.
        - Gradually decreases mutation rate and increases crossover rate over generations.
        - At the last generation, mutation is 0% and crossover is 100%.
        """
        Pc = (gen / ngen) ## Crossover increasing linearly 
        Pm = 1-(gen / ngen)   # # Mutation decreasing linearly DHM/ILC
        return Pc, Pm

     # Save the top 10 best final indivudal of population to a file
    def save_best_individuals(population, filename="best_individuals_DHM_ILC_LHA.csv"):
        top_individuals = tools.selBest(population, 50)  # Select the top 10 best individuals
    ...
    def save_best_10_fitness_per_gen(population, gen, filename="best_fitness_DHM_ILC_LHA.csv"):
        """
    ...
    def run_ga_mu_plus_lambda_dynamic(initial_mu, lambda_, ngen, min_mu, decrease_rate):
        ...
        for gen in range(ngen):
            ...
            mu = pop_size #max(min_mu, pop_size-round(pop_size*decrease_rate/100)) # new population size
        ...
        plt.savefig( 'pop_size_over_generation_DHM_ILC_LHA')
        ...
        # plt.legend()
        # plt.show()
        plt.savefig('pareto_front_DHM_ILC_LHA')
    ...
    run_ga_mu_plus_lambda_dynamic(initial_mu=50, lambda_=50, ngen=80, min_mu=40, decrease_rate=1)  # Example population of 20 and 10 generations lmbda_>= mu
